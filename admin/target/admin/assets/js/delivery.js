...
function listAllDeliveryCallBack(result) {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        let name = array[i].memberName == undefined ? " " : array[i].memberName;
        td += "<td style='vertical-align: middle;'>" + name + "</td>";
        let phone = array[i].memberPhone == undefined ? " " : array[i].memberPhone;
        td += "<td style='vertical-align: middle;'>" + phone + "</td>";
        let startAddress = array[i].startProvince + array[i].startCity + array[i].startDistrict + array[i].startAddress;
        td += "<td style='vertical-align: middle; width:500px;'>" + startAddress + "</td>";
        let endAddress = array[i].endProvince + array[i].endCity + array[i].endDistrict + array[i].endAddress;
        td += "<td style='vertical-align: middle; width:500px;'>" + endAddress + "</td>";
        let demand = (array[i].demandDistance / 1000).toFixed(2);
